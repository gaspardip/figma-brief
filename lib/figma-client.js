...
export class FigmaClient {
  ...
  /**
   * Fetch a screenshot via the Figma REST API (no MCP needed).
   * Requires figmaAccessToken and fileKey.
   * Falls back to MCP if REST is not configured.
   */
  async getScreenshotViaRest(nodeId, outputPath, fileKey) {
    const fk = fileKey ?? this.fileKey;
    const token = this.figmaAccessToken;

    if (!fk || !token) {
      return null;
    }

    const apiUrl = `https://api.figma.com/v1/images/${fk}?ids=${encodeURIComponent(nodeId)}&format=png&scale=2`;
    ...
    if (!response.ok) {
      this.log("Figma REST screenshot failed", {
    ...
    return this.downloadUrl(imageUrl, outputPath);
  }

  /**
   * Fetch comments for a file via the Figma REST API.
   * Returns an array of comment objects or empty array on failure.
   */
  async getComments(fileKey) {
    ...
    const apiUrl = `https://api.figma.com/v1/files/${fk}/comments`;
    ...
    try {
      ...
      if (!response.ok) {
        this.log("Figma REST comments failed", { status: response.status });
        return [];
      }
      ...
      this.log("Figma REST comments error", {
  ...
  /**
   * Get screenshot with automatic fallback: MCP → REST API.
   * Use this when you want to try MCP first but fall back to REST
   * if MCP is rate limited or unavailable.
   */
  async getScreenshotWithFallback(nodeId, outputPath, options = {}) {
    // Try MCP first (if connected)
    if (this.connected) {
      const result = await this.getScreenshot(
        nodeId,
        outputPath,
        options,
      ).catch(() => null);
      ...
      this.log("MCP screenshot failed, trying REST API", { nodeId });
    }

    // Fallback to REST API
    return this.getScreenshotViaRest(nodeId, outputPath, options.fileKey);
  }
}
