function createPromptBrief(brief) {
  return {
    ...
    visual: {
      ...
      frameScreenshots: brief.visual?.frameScreenshots ?? null,
      frameAnalysis: brief.visual?.frameAnalysis ?? null,
...
function buildStateSection(brief) {
  ...
  lines.push(
    ...
    "Map each transition as: trigger → state change → UI update → available actions.",
    "Example: `user clicks 'learn more' on toast → modal opens → canvas blocked, focus trapped → user can dismiss or take action`",
    "",
  );
...
function buildAccessibilitySection(brief) {
  ...
  lines.push(
    ...
    "- For non-standard interactive elements (custom hover effects, drag interactions, magnifiers), ensure keyboard equivalents exist and state changes are announced to screen readers",
    "",
  );
...
function buildFrameScreenshotsSection(brief) {
  ...
  const lines = [
    ...
    "",
    "Individual screen states at full resolution. Study EACH screenshot carefully before planning.",
    "Each represents a different state of the feature — match them exactly.",
    "",
  ];

  const frameAnalysis = brief.visual?.frameAnalysis ?? {};

  for (const [nodeId, frame] of Object.entries(frameScreenshots)) {
    if (frame.path) {
      lines.push(
      ...
      const analysis = frameAnalysis[nodeId];
...
function buildDesignerContextSection(brief) {
  ...
  const cleanContextNotes = contextNotes.filter((n) => {
    // Flow steps are kept regardless of length (they represent user journey steps)
    if (n.kind === "flow-step") {
      return n.text.trim().length > 5;
    }
...
function buildDataContractSection(brief) {
  ...
  const lines = [
    ...
    "4. **State management** — where the feature state lives (store, composable, local). Follow the existing state management pattern in the module you're integrating into",
    "",
  ];
...
export function buildPrompt(brief) {
  const lines = [
    ...
    "",
    "**First step:** Read the reference screenshot(s) below — both the overview and any per-state screenshots — and describe what you see:",
    ...
    "",
    "Your plan MUST explicitly address each of these before you write any code:",
    "",
    "- [ ] All UI states listed below with transitions between them",
    "- [ ] Component contracts (props, events) for every new component",
    "- [ ] Which existing components are reused (from the matches below)",
    "- [ ] i18n keys for every feature-specific user-visible string",
    ...
    "",
    "**Present your plan and WAIT for approval before writing any code.**",
    "",
    "Then implement following the sections below IN ORDER.",
  ...
  lines.push(...buildFrameScreenshotsSection(brief));
