...
function buildTextSection(brief) {
  ...
  const lines = [
    ...
    "",
    "Every text string below MUST be implemented as an i18n key, not hardcoded.",
    "Create a translation namespace for this feature and map each string.",
    "",
  ];
...
function buildDesignerContextSection(brief) {
  ...
  if (inferredNotes.length > 0) {
    ...
    for (const note of inferredNotes) {
      lines.push(`- ${note.text}`);
    }
...
export function buildPrompt(brief) {
  const lines = [
    ...
    "## Instructions",
    "",
    "Implement this design following the sections below IN ORDER.",
    "Each section marked REQUIRED must be fully addressed in your plan before writing code.",
